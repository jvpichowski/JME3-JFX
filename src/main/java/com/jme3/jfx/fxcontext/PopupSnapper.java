...
public class PopupSnapper {
	...
	BaseContext jmeFXcontainerReference;
	private Semaphore  repaintLock = new Semaphore(1);

	public PopupSnapper(final BaseContext containReference, final Window window, final Scene scene) {
		this.window = window;
	...
	public void paint(final IntBuffer buf, final int pWidth, final int pHeight) {
			...
			if ( lock ) {
			    try {
    				...
    				final int xoff = (int) this.window.getX() ;
    				final int yoff = (int) this.window.getY() ;
